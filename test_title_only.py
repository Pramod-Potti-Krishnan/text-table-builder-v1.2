#!/usr/bin/env python3
"""
Test only the improved title slide.
"""
...
async def test_title_slide():
    """Test improved title slide with image generation."""
    print("\n" + "=" * 80)
    print("Testing IMPROVED Title Slide")
    print("=" * 80)
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        # Save response
        with open(OUTPUT_DIR / "title_improved_response.json", "w") as f:
            json.dump(result, f, indent=2)

        # Save HTML
        if "content" in result:
            with open(OUTPUT_DIR / "title_improved.html", "w") as f:
        ...
        print(f"\n✓ Files saved:")
        print(f"  - {OUTPUT_DIR}/title_improved_response.json")
        print(f"  - {OUTPUT_DIR}/title_improved.html")
...
async def create_preview_html():
    ...
    title_html = (OUTPUT_DIR / "title_improved.html").read_text()

    # Read the JSON response to get background image
    with open(OUTPUT_DIR / "title_improved_response.json") as f:
...
<head>
    ...
    <title>Improved Title Slide Preview</title>
...
"""
    ...
    # Save preview HTML
    preview_path = OUTPUT_DIR / "title_improved_preview.html"
...
async def main():
    """Run test."""
    print("\n" + "=" * 80)
    print("IMPROVED TITLE SLIDE - LOCAL TESTING")
    print("=" * 80)

    try:
        # Test title slide
        await test_title_slide()
