#!/usr/bin/env python3
"""
Generate All Asymmetric Templates - Disable Model Routing
"""
import sys
import os
import requests
...
from app.services.llm_service import create_llm_callable
...
def main():
    print("=" * 80)
    print("ASYMMETRIC TEMPLATES - Using Flash Model Only")
    print("=" * 80)
    
    # Create LLM with model routing DISABLED
    llm_callable = create_llm_callable(enable_model_routing=False)
    ...
    for i, (variant_id, description) in enumerate(ASYMMETRIC_VARIANTS.items(), 1):
        ...
        slide_spec = {
            ...
            "slide_purpose": f"Visual review of {variant_id} layout structure",
            "key_message": "Asymmetric layout with colored sections and sidebar",
            "tone": "professional",
