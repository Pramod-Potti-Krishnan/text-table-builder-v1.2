        ...
        if component_id == "colored_section":
            ...
            if list_style == "numbers":
                ...
                for i in range(1, items_per_instance + 1):
                    margin = "0" if i == items_per_instance else "8px"
                    items_html += f'<li style="margin-bottom: {margin};">{{item_{i}}}</li>'
                list_html = f'<ol style="margin: 0; padding-left: 0; width: 100%; font-size: {BODY_FONT_SIZE}; line-height: {BODY_LINE_HEIGHT}; color: var(--text-primary, {{item_color}}); text-align: left; list-style-position: inside;">{items_html}</ol>'
                ...
                for i in range(1, items_per_instance + 1):
                    margin = "0" if i == items_per_instance else "8px"
                    items_html += f'<li style="margin-bottom: {margin};">{{item_{i}}}</li>'
                list_html = f'<ul style="list-style-type: disc; margin: 0; padding-left: 0; width: 100%; font-size: {BODY_FONT_SIZE}; line-height: {BODY_LINE_HEIGHT}; color: var(--text-primary, {{item_color}}); text-align: left; list-style-position: inside;">{items_html}</ul>'
